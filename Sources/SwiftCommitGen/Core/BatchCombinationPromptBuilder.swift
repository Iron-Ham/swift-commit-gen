...
struct BatchCombinationPromptBuilder {
  func makePrompt(metadata: PromptMetadata, partials: [BatchPartialDraft]) -> PromptPackage {
    ...
    let systemPrompt = Instructions {
      """
      You are an AI assistant merging multiple partial commit drafts into a single, well-structured commit message. Preserve all important intent from the inputs, avoid redundancy, and keep the final subject concise (<= 50 characters). Provide an optional body only when useful for additional context. The title should succinctly describe the change. If a body is present, it should describe the _purpose_ of the change, not just _what_ was changed.
      """
    }
